...
class Thread extends Eloquent
{
    ...
    *
    * @var null|Models::user()|Models::destint()
    */
    ...
    public function creator()
    {
            ...
            if($firstMessage->type==1)
            {
                $this->creatorCache = dentist::find($firstMessage->user_id);
            }else if($firstMessage->type==2){
                $this->creatorCache = user::find($firstMessage->user_id) ;
            }
    ...
    public function participantsString($userId = null, $columns = ['name'])
    {
        ...
        $dentistsTable = Models::table('dentists');
        $userPrimaryKey = Models::user()->getKeyName();

        $selectString = $this->createSelectString($columns);

        $participantNames = $this->getConnection()->table($participantsTable)
           
            ->join($usersTable,function($join){
                $join->on( "users" . '.id' ,"=","lexx_participants" . '.user_id')
                    ->on('lexx_participants'.'.type',"=",$this->getConnection()->raw('2'));
            })
            
            ->where($participantsTable . '.thread_id', $this->id)
            ->select($this->getConnection()->raw('users.name as name'));

            $participantNames1 = $this->getConnection()->table($participantsTable)
           
            ->join($dentistsTable,function($join){
                $join->on( 'dentists' . '.id',"=",'lexx_participants' . '.user_id')
                    ->on('lexx_participants'.'.type',"=",$this->getConnection()->raw('1'));
            })
            ->where($participantsTable . '.thread_id', $this->id)
            ->select($this->getConnection()->raw('dentists.name as name'));
        ...
        $name= $participantNames1->implode('name', ', ');
        $name.=', '. $participantNames->implode('name', ', ');
